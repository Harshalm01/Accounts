...
export default function Invoice() {
  ...
  const handleSaveFields = async (inv: InvoiceItem) => {
    try {
      // Build clean payload: only send fields that exist in the field maps
      const validKeys = new Set([...Object.keys(GST_FIELDS), ...Object.keys(NON_GST_FIELDS)]);
      const payload: Record<string, string> = {};
      for (const [key, val] of Object.entries(editingFields)) {
        if (validKeys.has(key) && key !== 'signature') {
          payload[key] = val;
        }
      }
  ...
  const renderScanResults = (inv: InvoiceItem) => {
    const fields = inv.type === 'GST' ? GST_FIELDS : NON_GST_FIELDS;
      ...
      <div className="px-8 py-6">
          ...
          <div className="space-y-4">
                          ...
                          {folderMenuOpen === f && (
                            <div className="absolute right-0 mt-1 w-36 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg shadow-lg py-1 z-20">
                              <button
                                onClick={async (e) => {
                                  e.stopPropagation();
                                  if (!confirm(`Delete folder "${f}"? Invoices inside will be moved to Uncategorized.`)) {
                                  ...
                                  try {
                                    ...
                                    if (res.ok) {
                                      ...
                                      fetchInvoices();
                                      fetchAllInvoices();
                                      if (filterFolder === f) setFilterFolder('all');
                                    } else {
            ...
            {loading ? (
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 p-12 text-center">
                ...
                <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">No invoices found</h3>
                <p className="text-gray-500 dark:text-gray-400">No invoices match the current filters</p>
              </div>
            ) : (
              <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-zinc-800">
                    {invoices.map(inv => (
                      <tr key={inv.id} onClick={() => { setSelectedInvoice(inv); setApproveFolder(inv.folder); setApproveNewFolder(''); setMoveFolder(inv.folder); setMoveNewFolder(''); setIsEditing(false); }} className="hover:bg-gray-50 dark:hover:bg-zinc-800/50 cursor-pointer transition-colors">
          ...
          return (
            <div className="space-y-4">
              <div className="flex flex-wrap gap-3 items-center">
                ...
                <div className="ml-auto flex items-center gap-3">
                  ...
                  {historyInvoices.length > 0 && (
                    <button
                      onClick={async () => {
                        if (!confirm('Are you sure you want to clear all invoice history? This will permanently delete all uploaded, scanned & rejected invoices and their files.')) return;
                        try {
                          const res = await fetch(`${API_URL}/api/invoices/clear-all`, { method: 'DELETE', headers: getAuthHeaders() });
                          if (res.ok) {
                            const data = await res.json();
                            alert(`Cleared ${data.count} invoice(s)`);
                            fetchInvoices();
                            fetchAllInvoices();
                            fetchFolders();
                          } else {
                        ...
                        } catch (err) { console.error('Clear history error:', err); alert('Failed to clear history'); }
                      }}
              ...
              {loading ? (
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 p-12 text-center">
                  <div className="text-5xl mb-4">📜</div>
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">No history</h3>
                  <p className="text-gray-500 dark:text-gray-400">No uploaded, scanned or rejected invoices found</p>
                </div>
              ) : (
                <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 overflow-hidden">
                  <table className="w-full text-sm">
        ...
        {selectedInvoice && (
          <div className="space-y-6">
            <button onClick={() => { setSelectedInvoice(null); setIsEditing(false); }} className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 flex items-center gap-1">
