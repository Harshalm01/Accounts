  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setUploadMessage({
          type: 'success',
          text: `File uploaded successfully! ${data.roaster.rowCount} rows processed.`
        });
        // Reset form
        setSelectedMonth('');
        setCsvFile(null);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-black">
      ...
      <div className="px-8">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-800 overflow-hidden mb-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500 mt-2">Supported formats: CSV, Excel (.csv, .xls, .xlsx)</p>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-800 overflow-hidden">
          <div className="bg-black px-8 py-5">
            ...
            <p className="text-gray-300 mt-1 text-sm">View and manage your uploaded roaster files</p>
          </div>
      ...
      {viewingRecord && (
        <div onClick={() => setViewingRecord(null)} className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg shadow-2xl w-full max-w-7xl max-h-[90vh] overflow-hidden flex flex-col">
            {/* Modal Header */}
            <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-5">
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider bg-gray-50 sticky top-0">
                            #
                          </th>
                          {Object.keys(viewingRecord.data[0]).map((header) => (
                            <th key={header} className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider bg-gray-50 sticky top-0">
                              {header}
                            </th>
                          ))}
                        </tr>
